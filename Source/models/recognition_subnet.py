...
class RecSubNet(nn.Module):
    ...
    def __init__(self, in_planes):
        ...
        self.conv2 = conv_bn_relu(conv_out_planes[0], conv_out_planes[1], 3, 1, 1)
        self.pool1 = nn.MaxPool2d((2, 2), (2, 2), (0, 0))  # 8 x 32

        self.conv3 = conv_bn_relu(conv_out_planes[1], conv_out_planes[2], 3, 1, 1)
        self.conv4 = conv_bn_relu(conv_out_planes[2], conv_out_planes[3], 3, 1, 1)
        self.pool2 = nn.MaxPool2d((2, 1), (2, 1), (0, 0))  # 4 x 32

        self.conv5 = conv_bn_relu(conv_out_planes[3], conv_out_planes[4], 3, 1, 1)
        self.conv6 = conv_bn_relu(conv_out_planes[4], conv_out_planes[5], 3, 1, 1)
        self.pool3 = nn.MaxPool2d((2, 1), (2, 1), (0, 0))  # 2 x 32
        self.conv7 = conv_bn_relu(conv_out_planes[5], conv_out_planes[6], 2, 1, 0)  # 1 x 31
        ...
        self.rnn = nn.LSTM(conv_out_planes[5], conv_out_planes[5], bidirectional=True, num_layers=1, batch_first=True)
    ...
    def forward(self, x):
        cnn_feat = self.cnn(x)
        cnn_feat = cnn_feat.squeeze(axis=2)  # [N, c, w]
